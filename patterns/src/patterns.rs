...
impl Patterns {
  ...
  pub fn find(&self, field: &Field, player: Player, first_match: bool) -> Vec<Pos> {
    ...
    if self.dfa.is_empty() || width < self.min_size - 2 || height < self.min_size - 2 {
      return Vec::new();
    }
    ...
    for y in left_border..height as i32 - right_border {
      for x in left_border..width as i32 - right_border {
