...
impl<N: Float + Sum + Copy> Search<N> {
  ...
  /// count when weighting its contribution to the bucket. `free_prop` is the
  /// fraction of a node's bucket contribution that is removed when the node
  ...
  pub async fn mcgs<M: Model<N>, R: Rng>(
    ...
    leafs.retain(|(path, terminal)| {
      ...
      let result = if *terminal || field.is_game_over(red_komi_x_2) {
        // Terminal nodes get no bias correction.
