  ...
  fn select_edge(&self, node_idx: usize, noise: bool) -> Option<usize> {
    ...
    for (idx, edge) in node.children.iter().enumerate() {
      ...
      let q = if total_edge_visits > 0 {
        ...
        // as in KataGo. With little mass visited yet the parent's running average
        // rests on few playouts, so it is blended towards the raw net value with
        // the weight `min(1, mass²)` (KataGo's `fpuParentWeightByVisitedPolicy`
        // with pow 2).
